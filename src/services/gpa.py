...
def get_list(dict: Dict[str, str]) -> List:
...
def grade_to_classification(gpa: float) -> str:
    """Transform a GPA to it's corresponding level.
...
def gpa_to_grades(gpa: float, course_num: int) -> str:
    ...
    grade = get_list(main_grades)  # Get the grades (A to F)
    grades_needed = {}

    while True:
        temp += main_grades[grade[i]] / course_num  # Add grade point of grade to temp

        if (
            temp > gpa and i > 5
        ):  # E and F carry zero weight. So end loop when grade is on 5(E) and temp > gpa
            break

        if (
            temp > gpa
        ):  # If Grade[i] can't be added without exceeding GPA, delete added value and rollover to next grade.
...
def calculate_fgpa(fgpa_data: Fgpa) -> Dict[str, str]:
    cgpa1 = fgpa_data.cgpa1
    cgpa2 = fgpa_data.cgpa2
    cgpa3 = fgpa_data.cgpa3
    cgpa4 = fgpa_data.cgpa4
...
def calculate_min_max_cgpa(data: MinMaxCgpa) -> Dict[str, str]:
    old_chours = data.old_chours
    new_chours = data.new_chours
    old_cgpa = data.old_cgpa
    ...
    try:
        ...
        old_points = (
            old_chours * old_cgpa
        )  # Weight of current gpa over current credit hours.

        max_cgpa = (
            (new_chours * 4) + old_points
        ) / total_chours  # Weight of new gpa over sem's credit hours/ divided by total credit hours.
        ...
        return {
            "old_cgpa": str(old_cgpa),
            "max_cgpa": str(max_cgpa),
            "min_cgpa": str(min_cgpa),
            "classification_max_cgpa": levels_max_cgpa,
            "classification_min_cgpa": levels_min_cgpa,
        }
...
def calculate_new_gpa(cgpa_data: Cgpa) -> Dict[str, str]:
    grades_list = cgpa_data.grades
    credits_list = cgpa_data.credit
    ...
    try:
        ...
        for i in range(len(grades_list)):
            ...
            total_gpt += grades_list[i] * credits_list[i]  # Total grade point
        ...
        )  # Raise a generic exception to be caught in the endpoint


def calculate_req_grades(data: RequiredGrades) -> Dict[str, str]:
    old_cgpa = data.old_cgpa
    old_chours = data.old_chours
    new_cgpa = data.new_cgpa
    new_chours = data.new_chours
    ...
    try:
        ...
        new_points = (
            total_chours * new_cgpa
        )  # Average weight of your desired gpa over your total course hours.
        old_points = (
            old_chours * old_cgpa
        )  # Average weight of your current gpa over your current course hours.
        diff_points = (
            new_points - old_points
        )  # Difference between your desired weight and your current weight.
        ...
        res3 = f"For the data provided, you'll need a {gpa} GPA which requires you to fail all courses."
        res4 = f"For this info, you'll need a {gpa} GPA which requires minimum grade(s) of {req_grades}"

        if gpa > 4.00:
            return {"feedback": res1}
        if gpa < 0.00:
            return {"feedback": res2}
        if len(req_grades) == 0:
            return {"feedback": res3}
